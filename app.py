...
# User session data
user_data = {"name": "", "age": "", "step": 0, "total": 0.0, "order": [], "reservation_step": 0}
...
def chat():
    ...
    # Reservation flow takes priority
    if user_data.get("reservation_step", 0) > 0:
        response = handle_reservation(user_message)
    else:
        ...
        if step == 0:
            ...
            response = handle_menu_selection(choice)
...
def show_menu():
    ...
    return (
        ...
        "<button onclick=\"sendMessage('4')\">4. Make a Reservation</button><br>"
...
def handle_menu_selection(choice):
    ...
    if choice in ["1", "menu"]:
        ...
        # Reset user data
        user_data.update({"name": "", "age": "", "step": 0, "total": 0.0, "order": [], "reservation_step": 0})
        return f"👋 Thank you, {name}! Have a flavorful day! 🍛"

    else:
        return "I didn’t understand that. Please choose a number (1–5)."


def handle_reservation(message):
    """Step-by-step reservation flow."""
    step = user_data.get("reservation_step", 0)
    ...
    if step == 1:
        ...
        return (
            ...
            f"Thank you for choosing Namaste Indian Kitchen — we look forward to serving you! 🍛<br><br>"
            + show_menu()
        )
